...
**
**--------------File Info---------------------------------------------------------------------------------
** File name:               s3c6410_h264_hwaccel.c
** Last modified Date:      2012-2-2
** Last Version:            1.0.0
** Descriptions:            H.264 硬件加速器
**
**--------------------------------------------------------------------------------------------------------
...
**--------------------------------------------------------------------------------------------------------
*********************************************************************************************************/
static int H264_StartFrame(AVCodecContext  *pCodecCtx,
...
/*********************************************************************************************************
** Function name:           H264_DecodeSlice
** Descriptions:            解码一个分片
** input parameters:        pCodecCtx           解码器上下文
**                          pucSlice            分片
**                          uiSliceSize         分片的大小
...
**--------------------------------------------------------------------------------------------------------
*********************************************************************************************************/
static int H264_DecodeSlice(AVCodecContext  *pCodecCtx,
    ...
    if (pucSlice == NULL && uiSliceSize == 0) {                         /*  关闭硬件加速器              */
        ...
        if (pHWAccelCtx->bConfig == FALSE) {                            /*  如果未配置 MFC              */
            ...
            iError = s3c_mfc_ioctl(NULL, &(pHWAccelCtx->fFd),
                                   S3C_MFC_IOCTL_MFC_H264_DEC_INIT, (unsigned long)&MFCParam);
...
**--------------------------------------------------------------------------------------------------------
*********************************************************************************************************/
static int H264_EndFrame(AVCodecContext  *pCodecCtx)
{
    HWAccelContext  *pHWAccelCtx = pCodecCtx->hwaccel_context;
    H264Context     *pH264Ctx    = pCodecCtx->priv_data;
    MpegEncContext  *pMpegEncCtx = &pH264Ctx->s;
    uint8           *pucYUVBuffer;
    s3c_mfc_args_t   MFCParam;
    int              iError;
    ...
    iError = s3c_mfc_ioctl(NULL, &(pHWAccelCtx->fFd), S3C_MFC_IOCTL_MFC_H264_DEC_EXE, (unsigned long)&MFCParam);
...
/*********************************************************************************************************
    H.264 硬件加速器
*********************************************************************************************************/
AVHWAccel ff_s3c6410_h264_hwaccel = {
        .name           = "s3c6410_h264_hwaccel",
        .type           = AVMEDIA_TYPE_VIDEO,
        .id             = CODEC_ID_H264,
        .pix_fmt        = PIX_FMT_YUV420P,
        .capabilities   = 0,
        .start_frame    = H264_StartFrame,
        .end_frame      = H264_EndFrame,
        .decode_slice   = H264_DecodeSlice,
        .priv_data_size = 0,
};
/*********************************************************************************************************
    END FILE
