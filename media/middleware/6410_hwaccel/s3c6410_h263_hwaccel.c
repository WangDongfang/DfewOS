...
**
**--------------File Info---------------------------------------------------------------------------------
** File name:               s3c6410_h263_hwaccel.c
** Last modified Date:      2011-3-25
** Last Version:            1.0.0
** Descriptions:            H.263 硬件加速器
**
**--------------------------------------------------------------------------------------------------------
...
**--------------------------------------------------------------------------------------------------------
*********************************************************************************************************/
static int H263_StartFrame(AVCodecContext  *pCodecCtx,
...
/*********************************************************************************************************
** Function name:           H263_DecodeSlice
** Descriptions:            解码一个分片
** input parameters:        pCodecCtx           解码器上下文
**                          pucSlice            分片
**                          uiSliceSize         分片的大小
...
**--------------------------------------------------------------------------------------------------------
*********************************************************************************************************/
static int H263_DecodeSlice(AVCodecContext  *pCodecCtx,
    ...
    if (pucSlice == NULL && uiSliceSize == 0) {                         /*  关闭硬件加速器              */
        ...
        if (pHWAccelCtx->bConfig == FALSE) {                            /*  如果未配置 MFC              */
            ...
            iError = s3c_mfc_ioctl(NULL, &(pHWAccelCtx->fFd), 
                                   S3C_MFC_IOCTL_MFC_H263_DEC_INIT, (unsigned long)&MFCParam);
...
**--------------------------------------------------------------------------------------------------------
*********************************************************************************************************/
static int H263_EndFrame(AVCodecContext  *pCodecCtx)
{
    HWAccelContext  *pHWAccelCtx = pCodecCtx->hwaccel_context;
    ...
    iError = s3c_mfc_ioctl(NULL, &(pHWAccelCtx->fFd), S3C_MFC_IOCTL_MFC_H263_DEC_EXE, (unsigned long)&MFCParam);
...
/*********************************************************************************************************
    H.263 硬件加速器
*********************************************************************************************************/
AVHWAccel ff_s3c6410_h263_hwaccel = {
        .name           = "s3c6410_h263_hwaccel",
        .type           = AVMEDIA_TYPE_VIDEO,
        .id             = CODEC_ID_H263,
        .pix_fmt        = PIX_FMT_YUV420P,
        .capabilities   = 0,
        .start_frame    = H263_StartFrame,
        .end_frame      = H263_EndFrame,
        .decode_slice   = H263_DecodeSlice,
        .priv_data_size = 0,
};
/*********************************************************************************************************
    END FILE
