...
int s3c_mfc_get_config_params(s3c_mfc_inst_context_t *mfc_inst, s3c_mfc_args_t *args )
{
	...
	case S3C_MFC_GET_CONFIG_DEC_H264_CROPINFO:
		args->get_config.out_config_value[0] = mfc_inst->crop_value0;
		args->get_config.out_config_value[1] = mfc_inst->crop_value1;
		ret = S3C_MFC_INST_RET_OK;
		break;
...
int s3c_mfc_set_config_params(s3c_mfc_inst_context_t *mfc_inst, s3c_mfc_args_t *args)		
{
	...
	case S3C_MFC_SET_CONFIG_ENC_VOPTIMEINCRESOL :
		mfc_debug("S3C_MFC_SET_CONFIG_ENC_VOPTIMEINCRESOL : vop_time_ince_sol = %d\n",  args->set_config.in_config_value[0]);
		mfc_inst->vop_time_ince_sol = args->set_config.in_config_value[0];
		ret = S3C_MFC_INST_RET_OK;
		break;

	case S3C_MFC_SET_CONFIG_ENC_LEVEL:
		mfc_debug("S3C_MFC_SET_CONFIG_ENC_VOPTIMEINCRESOL : mpeg4_level = %d\n",  args->set_config.in_config_value[0]);
		mfc_inst->mpeg4_level = args->set_config.in_config_value[0];
		ret = S3C_MFC_INST_RET_OK;
		break;
