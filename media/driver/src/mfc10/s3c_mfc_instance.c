...
int s3c_mfc_inst_init_dec(s3c_mfc_inst_context_t *ctx, s3c_mfc_codec_mode_t codec_mode, unsigned long strm_leng)
{
	...
	if (ctx->codec_mode == AVC_DEC) {
		ctx->crop_value0 = readl(s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_RET_DEC_SEQ_CROP_LEFT_RIGHT);
		ctx->crop_value1 = readl(s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_RET_DEC_SEQ_CROP_TOP_BOTTOM);
	}
...
int s3c_mfc_instance_init_enc(s3c_mfc_inst_context_t *ctx, s3c_mfc_codec_mode_t codec_mode, s3c_mfc_enc_info_t *enc_info)
{
...
#if (S3C_MFC_USE_USER_GAMMA_VALUE == 1)	
	int user_gamma_value;
#endif

#if (S3C_MFC_USE_USER_QPMAX_VALUE == 1)
	int user_qpmax_value;
#endif
	...
	if ((ctx->vop_time_ince_sol > 0)&&(ctx->vop_time_ince_sol < 30000))
	{
		ctx->frame_rate_division = (ctx->vop_time_ince_sol / ctx->frame_rate_residual) - 1;
		ctx->frame_rate_residual = ctx->vop_time_ince_sol;
	}
	...
	writel(S3C_MFC_RC_ENABLE | (ctx->bitrate << 1) | (SKIP_DISABLE), 				 \
...
#if (S3C_MFC_USE_USER_GAMMA_VALUE == 0)
	/* Default value = 0x1000 */
	writel(S3C_MFC_USE_GAMMA_DISABLE, s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_ENC_SEQ_RC_OPTION);
#else
	writel(S3C_MFC_USE_GAMMA_ENABLE, s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_ENC_SEQ_RC_OPTION);
	/* GAMMA value 0 to 0x7FFFF  if you set high gamma value then you can lower maximum bitrate */
	user_gamma_value = 0x6000;  /* Here you can set specific value */
	writel(user_gamma_value, s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_ENC_SEQ_RC_GAMMA);
#endif

	switch(ctx->codec_mode) {
		case MP4_ENC:
			writel(S3C_MFC_MPEG4_ENCODE, s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_ENC_SEQ_COD_STD);
			writel(~(0x1F), s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_ENC_SEQ_MP4_PARA);	
			writel(S3C_MFC_DATA_PART_DISABLE | S3C_MFC_DATA_VERID_1, s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_ENC_SEQ_MP4_PARA);	
...
#if (S3C_MFC_USE_USER_QPMAX_VALUE == 0)
	/* MaxQp H.264 default value is  40, MPEG-4/H.263 default value is 28 */
	writel(S3C_MFC_USER_QP_MAX_DISABLE, s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_ENC_SEQ_RC_OPTION);
#else
	writel(S3C_MFC_USER_QP_MAX_ENABLE, s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_ENC_SEQ_RC_OPTION);
	/* if you set lower qp max value then i frame size will be smaller and p frame size will be bigger */
	user_qpmax_value = 20;  /* Here you can set specific value */
	writel(user_qpmax_value, s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_ENC_SEQ_RC_QP_MAX);
#endif
...
{
	int hdr_size, hdr_size2, hdr_vos_size, hdr_vo_size, hdr_vol_size;
	int size;
	...
	if (S3C_MFC_INST_STATE_CHECK(ctx, S3C_MFC_INST_STATE_ENC_INITIALIZED)) {
		if (ctx->codec_mode == MP4_ENC) {
			/*  ENC_HEADER command  */
			s3c_mfc_inst_enc_header(ctx, 1, 0, ctx->phys_addr_stream_buffer, ctx->stream_buffer_size, \
												&hdr_vos_size);	// VOS

			s3c_mfc_inst_enc_header(ctx, 2, 0, ctx->phys_addr_stream_buffer+(hdr_vos_size+3), \
								ctx->stream_buffer_size -(hdr_vos_size+3), \
											&hdr_vo_size);	// VO

			s3c_mfc_inst_enc_header(ctx, 0, 0, ctx->phys_addr_stream_buffer+(hdr_vos_size+3)+(hdr_vo_size+3), \
								ctx->stream_buffer_size-(hdr_vos_size+3)-(hdr_vo_size+3), \
												&hdr_vol_size);	// VOL
			hdr_size = hdr_vos_size+hdr_vo_size +3 +hdr_vol_size+3;
												...
												int outbuf_size, int *hdr_size)
{
	...
	if ((ctx->codec_mode == MP4_ENC) && (hdr_code == 1)) {
		encode_level = ((ctx->mpeg4_level<<8) | hdr_code);
	} else {
		encode_level = hdr_code;
	}


	/* Set the address of each component of YUV420 */
	writel(encode_level, s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_ENC_HEADER_CODE);
											...
											unsigned int param_change_val)
{
	...
	if (param_change_enable == (1 << 0)) { /* gop number */
		...
		ctx->frame_rate_division = ctx->frame_rate_residual/param_change_val - 1;
	    
		writel((ctx->frame_rate_division << 16) | ctx->frame_rate_residual, s3c_mfc_sfr_base_virt_addr + S3C_MFC_PARAM_ENC_CHANGE_F_RATE);
		ctx->enc_change_framerate = 1;	
