...
int s3c_mfc_open(struct inode *inode, struct file *file)
{
...
int s3c_mfc_release(struct inode *inode, struct file *file)
{
...
int s3c_mfc_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	int ret = 0;
	int buf_size;
...
dma_addr_t s3c_mfc_mmap(struct file *filp)
{
	...
	return S3C_MFC_BASEADDR_DATA_BUF;
}
...
int s3c_mfc_probe(struct platform_device *pdev)
{
...
#define S3C6400_PA_MFC          (0x7E002000)                            /*  MFC SFR 区域的物理基地址    */
#define S3C_SZ_MFC              (4096)                                  /*  MFC SFR 区域的大小          */
	s3c_mfc_sfr_base_virt_addr = ioremap_nocache(S3C6400_PA_MFC, S3C_SZ_MFC - 1);
    ...
    #define VIC_CHANNEL_MFC     16
	ret = request_irq(VIC_CHANNEL_MFC, s3c_mfc_irq, IRQF_DISABLED, pdev->name, pdev);
	if (ret != 0) {
...
static struct platform_device GpdS3Mfc = {
    .name = "s3c-mfc",
};
...
int __init s3c_mfc_init(void)
{
    ...
    init_waitqueue_head(&s3c_mfc_wait_queue)

    return s3c_mfc_probe(&GpdS3Mfc);
}
