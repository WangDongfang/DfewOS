...
BOOL s3c_mfc_init_yuvbuf_mgr(unsigned char *buffer_base, int buffer_size)
{
	...
	s3c_mfc_segment_info = s3c_mfc_get_segment_info_virt_addr();
	...
	s3c_mfc_commit_info = s3c_mfc_get_commit_info_virt_addr();
	for (i = 0; i < s3c_mfc_num_segments; i++) {
		s3c_mfc_commit_info[i].index_base_seg  = -1;
		s3c_mfc_commit_info[i].num_segs        = 0;
	}

	mfc_debug("address of segment_info=%x, address of commit_info =%x\n",s3c_mfc_segment_info,s3c_mfc_commit_info);
...
{
	mfc_debug("s3c_mfc_yuv_buffer_mgr_final\n");
